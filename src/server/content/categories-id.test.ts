...
import { GET as GETHandler, PUT as PUTHandler, DELETE as DELETEHandler } from './categories-id'

const params = { id: 'cat-1' }
const GET = (request: Request) => GETHandler({ request, params } as never)
const PUT = (request: Request) => PUTHandler({ request, params } as never)
const DELETE = (request: Request) => DELETEHandler({ request, params } as never)
...
describe('GET /api/content/categories/[id]', () => {
  ...
  it('returns auth error', async () => {
    ...
    const res = await GET(createNextRequest('/api/content/categories/cat-1'))
