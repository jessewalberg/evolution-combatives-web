...
import { GET as GETHandler } from './videos-id'

const GET = (request: Request, id: string) => GETHandler({ request, params: { id } } as never)
...
describe('GET /api/content/videos/[id]', () => {
  ...
  it('returns auth error', async () => {
    ...
    const res = await GET(createNextRequest('/api/content/videos/v1'), 'v1')
  ...
  it('returns 404 when video not found', async () => {
    ...
    const res = await GET(createNextRequest('/api/content/videos/missing'), 'missing')
    const body = await res.json()
